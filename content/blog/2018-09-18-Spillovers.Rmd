...
There are many different estimands that allow for spillovers, but which correspond to the average treatment effect in the absence of spillovers as a specific case. For instance, we can define the difference for a unit when it---and only it---is treated, compared to a situation in which no unit is treated. In potential outcomes notation that could be written, for unit 3, say, as:
...
dgp <- function(i, Z, G) mean(Z[G == G[i]]) ^ gamma + rnorm(1,0,sd_i)
...
reveal_Y <- declare_reveal(Y = sapply(1:N,function(i) dgp(i, Z, G)),
                           handler = fabricate)
estimator <- declare_estimator(Y ~ Z,
                               estimand = estimand,
                               model = lm_robust,
                               label = "naive")
simple_spillover_design <- population + estimand + assignment +
  reveal_Y + estimator
```
...
kable(reshape_diagnosis(diagnosis)[,-(2:3)], digits = 2)
```
...
Here is an alternative design that implements the original design on a sample of one subject per group:

```{r, error = TRUE, warning = FALSE}
sparse_design <- insert_step(simple_spillover_design, after = "Treat_1",
...
Getting an unbiased result by using sparser data depended on having a good understanding of the nature of spillovers. In this case, we made use of the fact there would be no spillovers between groups.
