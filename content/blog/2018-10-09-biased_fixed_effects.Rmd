...
```{r}
within_block <- one_draw %>%
  group_by(block) %>%
  summarise(block_ATE     = mean(Y_Z_1 - Y_Z_0),
            block_ATE_est = mean(Y[Z == 1]) - mean(Y[Z == 0]),
            size          = n(),
            var_p         = mean(Z) * (1 - mean(Z))) %>% 
  mutate(sample_weight    = size / sum(size),
         fe_weight        = (size*var_p) / sum(size*var_p))
```
```{r,echo = FALSE}
