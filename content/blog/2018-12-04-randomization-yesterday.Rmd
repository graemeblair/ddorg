...
here_you_go <- fabricate(
  school = add_level(N = 20),
  class  = add_level(
    N = 4, 
    treatment = block_ra(blocks = school)))
```
...
Set up like this it is also easy to implement the assignment multiple times:

```{r}
many_assignments <- replicate(5, draw_data(design))
```

Some tools in the `randomizr` package give some extra bonuses. If you "declare" the randomization you can use the declaration to get a nice summary of the procedure and also obtain a matrix of all of the possible random assignments (or a random subset, since the full set is usually very large!).

```{r, comment = ""}
classes_data <- make_data()
ra_declaration <- with(classes_data, declare_ra(blocks = class))

# learn about the procedure
summary(ra_declaration)

# generate permutations matrix
perm_mat <- obtain_permutation_matrix(ra_declaration, maximum_permutations = 200)
```
...
One advantage of a matrix like this is that it lets you see directly what the assignment probabilities are! This is useful in complex randomizations where different units might get assigned to treatment with quite different probabilities depending on their characteristics. 

```{r, comment = ""}
 summary(apply(perm_mat, 1, mean))
```
...
# Create better blocks from richer data 
...
# Saving the output to an excel spreadsheet
