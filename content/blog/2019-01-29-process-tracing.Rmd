...
date: "2019-01-29"
slug: "2019-01-29-process-tracing"
output:
  html_document:
    highlight: tango
    theme: cerulean
...
Qualitative process-tracing sometimes seek to answer "cause of effects" claims using within-case data: how probable is the hypothesis that $X$ *did in fact* cause $Y$? @fairfield2017explicit, for example, ask whether the right-wing changed position on tax reform during the 2005 Chilean presidential election ($Y$) *because* of anti-inequality campaigns ($X$) by examining whether the case study narrative bears evidence that you would only expect to see if this were true.^[For example: "the former President [said] that the tax subsidy 'never would have been eliminated if I had not taken [the opposition candidate] at his word' when the latter publicly professed concern over inequality."]  We declare a Bayesian process-tracing design and use it to think through choices about what kinds of within-case information have the greatest probative value.

Say we want to evaluate a case-specific hypothesis, $H$, regarding whether $Y$  happened *because* $X$ happened. The hypothesis is not that $X$ is the only cause of $Y$, but more simply whether $Y$ would have been different had $X$ been different. A researcher looks for "clues" or evidence, $E$, in a case narrative or other qualitative data, which would be more or less surprising to see depending on whether $H$ is true. @collier2011understanding lays out the basic strategy. In a beautiful recent paper, @murtas2017new show how to justify updating case-level inferences from experimental data on moderators and mediators.

Formally declaring and diagnosing such a procedure yields two nonobvious insights:

  1. **Straws in the wind ≥ Smoking guns**: strong but rare clues do not always give you better answers *on average* than weak but common clues.
  
  2. **The joint distribution of clues is really important**: Many applications of Bayesian process-tracing implicitly assume that clues are generated independently. Yet, when clues are negatively correlated (i.e., they arise through *alternative* causal pathways), they are jointly much more informative than when they are positively correlated. 
...
For the simulations, we will imagine we are in a world with 195 countries of which roughly 30% have natural resources ($X$) (that's easy, and observable to our imagined researcher). We will also specify a model in which civil war is governed by causal pathway 1 ($X$ causes $Y$) in roughly 20% of countries, by pathway 2 ($\neg X$ causes $Y$) in only 10% of countries, by pathway 3 ($Y$ irrespective of $X$) in 20% of countries, and by pathway 4 ($\neg Y$ irrespective of $X$) in half of all countries (that's not so easy for our imaginary researcher to know).

This gives us enough to put down the stub of a design in which a model generates data with these features, an imaginary researcher samples one case from the $X=Y=1$ group, and defines the question the researcher wants to ask about this case (notice here the inquiry is defined after the sampling). 
...
We now turn to the answer strategy. For this, we'll use Bayes' rule to figure out $Pr(H \mid E)$: the posterior probability that $X$ caused $Y$ in the case we chose, given the evidence we found. We make a function that calculates the posterior using Bayes'rule:
...
calculate_posterior <- function(data, p_H, p_E_H, p_E_not_H, label, result) {
  data.frame(posterior_H = p_E_H * p_H / (p_E_H * p_H + p_E_not_H * (1 - p_H)),
             estimator_label = label, estimand_label = "did_X_cause_Y", result = result)}
```

Bayes' rule makes use of the probability of observing $E$ if $H$ is true and the probability of  observing $E$ if $H$ is not true. The more different these probabilities are the more you learn from new data. Qualitative texts sometimes describe different tests based on the differences between these probabilities [@collier2011understanding]. Consider two:

* **A straw-in-the-wind test.** Let's say, for example, that $E_1$ is the national army taking control over natural resources during a civil war. Say we think that that's likely to happen if the natural resources caused the war: $Pr(E_1 \mid H) = .75$. But even if the natural resources didn't cause the war, the national army might still take over natural resources for other reasons, say $Pr(E_1 \mid \neg H) = .25$. This type of clue is often called a "straw-in-the-wind" (SIW): you expect to see it if $H$ is true, but it's not completely surprising to see it if $H$ is false.

* **A smoking gun test**. Say you were to look to see whether one of the antagonists was an armed group whose main name, aims, and ideology were centered around the capture and control natural resources. You might believe that observing such a clue is really unlikely overall, even if $H$ is true. But it's very informative if it is observed, since it's so unlikely to happen if $H$ is not true: it's a "smoking gun" (SMG). Let's say $Pr(E_2 \mid H) = .3, Pr(E_2 \mid \neg H) = 0.05$. 

We can define a step that generates clue observations given these probabilities. 

```{r}  
observe_clue <- 
  declare_step(
    SIW_observed = rbinom(n = N, size = 1, 
                          prob = ifelse(test = causal_process == 'X_causes_Y', .75, .25)),
    SMG_observed = rbinom(n = N,size = 1, 
                          prob = ifelse(test = causal_process == 'X_causes_Y', .3,  .05)),
    handler = fabricate, label = "Independent Clues")
```

We assume here that these clues are independent conditional on the causal process. This is a strong assumption: the fact that an armed group formed in order to take resources ($E_2$) might convince the government to take over the natural resource ($E_1$), or dissuade them! We therefore relax this "Independent Clues" assumption below.

We also need to specify the imaginary researcher's prior belief that $H$ is true. The imaginary researcher knows that only two processes, 1 and 3, could have generated the data $X = Y = 1$. Thus, they specify a "flat" prior: $Pr(H) = .5$.
...
design <-
  ...
  declare_estimator(
    result    = data$SIW_observed, 
    p_H       = .5, 
    p_E_H     = ifelse(test = data$SIW_observed, .75, .25),
    p_E_not_H = ifelse(test = data$SIW_observed, .25, .75),
    label     = "Straw in Wind",
    handler   = calculate_posterior) +
  
  declare_estimator(
    result    = data$SMG_observed, 
    p_H       = .5, 
    p_E_H     = ifelse(test = data$SMG_observed, .3,  .7),
    p_E_not_H = ifelse(test = data$SMG_observed, .05, .95),
    label     = "Smoking Gun",
    handler   = calculate_posterior)
```
...
As expected, the smoking gun can be highly probative: the bottom right panel shows that we sometimes get the answer almost exactly right (i.e., in those cases when $H$ is true and we observe a smoking gun). Note, however, that this is pretty rare: most of the time we guess $Pr(H\mid E_2) =$ `r round(custom_diags$mean_post_H_no_E[2],2)`, which is pretty far from the true values of 0 and 1. We sometimes, but very rarely,  get a false smoking gun. The smoking gun distribution is one that is centered close to the prior but sometimes makes big, and usually correct, inferential leaps.

By contrast, the left column shows that the straw-in-the-wind strategy gets pretty close pretty often. Whenever it is observed, the researcher guesses  $Pr(H\mid E_1) =$ `r round(custom_diags$mean_post_H_E[1],2)`, and when it is not they guess $Pr(H\mid E_1) =$ `r round(custom_diags$mean_post_H_no_E[1],2)`. This means they sometimes make mistakes -- they observe $E_1$ even when $H$ is false, and vice versa. But it happens rarely enough that they tend to move in the right direction most of the time. 

The net result is that in this case the straw-in-the-wind test has lower root mean squared error (RMSE): `r round(custom_diags$rmse[1],2)` vs. the smoking gun's `r round(custom_diags$rmse[2],2)`. It's less wrong more often. In general, however, which performs better depends on the particular probabilities assumed.
...
corr_design <- corr_design + 
  declare_estimator(
    result = data$SIW_SMG,
    p_H = .5, 
    p_E_H = joint_prob(.8, .3, rho)[[data$SIW_SMG]],
    p_E_not_H = joint_prob(.2, 0, rho)[[data$SIW_SMG]],
    label = "Update from both clues",
    handler = calculate_posterior)
```

```{r,echo=FALSE}
designs <- redesign(corr_design,rho = seq(-.3,.3,.1))
if(do_diagnosis){
