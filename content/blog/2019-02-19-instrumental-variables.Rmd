...
We often think of an instrumental variable ($Z$) as a random shock that generates exogenous variation in a treatment of interest $X$. The randomness of $Z$ lets us identify the effect of $X$ on $Y$, at least for units for which $Z$ perturbs $X$ ("compliers"), in a way that's not possible by just looking at the relationship between $X$ and $Y$.  But surprisingly, we think, the instrumental variables estimator can be consistent for the effect of $X$ on $Y$  even when the relationship between the instrument ($Z$) and the endogenous variable ($X$) is confounded [@hernan2006instruments]. That's the good news. Less good news is that when there is effect heterogeneity it can be hard to know *whose* average treatments effects get estimated [@Swanson2017]. We use a declaration and diagnosis to illustrate these points. We show first that instrumental variables approaches can do well even with confounded instruments if there are constant treatment effects. We then show that with a confounded instrument and effect heterogeneity  the interpretation of who the compliers are for "complier average effects" really depends on the nature of confounding. 

Consider the causal graph below (a directed acyclic graph, or DAG). As usual, $U_2$ confounds the relationship between $X$ and $Y$, but now a binary $U_1$ also confounds the relationship between $Z$ and $X$ (see for instance Fig 2d @swanson2018partial). To emphasize, in this set up, the instrumental variable and the **instrumented** variable share a common cause in $U_1$.
...
design <-
  ...
  # X potential outcomes in terms of u1 and Z (there are **four**)
  declare_potential_outcomes(X ~ as.numeric((U2 == "U1_c") * U1 +
                                            (U2 == "Z_c")  * Z),
                             assignment_variables = c(U1, Z)) +

  # X potential outcomes in terms of  Z given u1 (there are **two**)
  declare_potential_outcomes(X ~ as.numeric((U2 == "U1_c") * U1 +
                                            (U2 == "Z_c")  * Z),
  ...
  # Three estimators
  declare_estimator(Y ~ Z, estimand = "ITT", model = lm_robust, label =  "lm_robust_itt") +
  declare_estimator(X ~ Z, estimand = "first_stage", model = lm_robust, label =  "lm_robust_first_stage") +
...
1. The ATE is very badly estimated by the naive approach. It is, however, well-estimated by the instrumental variables estimator. This, despite the fact that:
...
Although we have described $Z$ as being the instrument, you can see in this DAG that both $Z$ and $U_1$ can affect $X$. So perhaps $U_1$ should of as an instrument even if it is unobserved? And perhaps compliers should be thought of as units that comply with $U_1$ and not $Z$? 

To explore this possibility, we explore design modifications that shift probability mass between two special cases of the DAG.
