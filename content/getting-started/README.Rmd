...
You can [read our paper](https://declaredesign.org/declare.pdf) for an overview of the ideas behind `DeclareDesign` -- including the Model-Inquiry-Data Strategy-Answer Strategy (MIDA) framework for describing designs and the declare-diagnose-redesign workflow for improving research designs before implementing them. For a shorter introduction, read our [MIDA guide](/mida). Then continue reading this guide to the DeclareDesign software.

We wrote `DeclareDesign` in the R statistical environment because of the availability of other tools for implementing research designs and because it is free-to-use. You could, however, implement the MIDA framework in any programming language. Indeed, a design could be declared in writing or mathematical notation and then diagnosed using analytical formula.^[However, analytical diagnoses may not be ideal for typical designs in the social sciences: they do not account for the specific features of research designs such as varying numbers of units per cluster and the interaction of choices about a data strategy and an answer strategy.]  We have [translated](https://declaredesign.org/pap) of core parts of the declaration and diagnosis process into Stata, Python, and Excel.
